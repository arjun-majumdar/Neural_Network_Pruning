...
loss = loss_fn(predictions = preds, targets = targets)

# Back-propagate the 'average' gradient of this mini-batch-
loss.backward()
...
# data.unsqueeze(0).shape, targets.unsqueeze(0).shape
# (torch.Size([1, 64, 1, 28, 28]), torch.Size([1, 64]))
...
# type(per_sample_grads), len(per_sample_grads)
# (list, 8)
...
for i in range(len(per_sample_grads)):
    print(f"i: {i}, shape: {per_sample_grads[i].shape},"
          f" GNSR = {per_sample_grads[i].mean() / per_sample_grads[i].var():.4f}"
          )
'''    
i: 0, shape: torch.Size([64, 32, 1, 3, 3]), GNSR = -0.3272
i: 1, shape: torch.Size([64, 32]), GNSR = -2.9507
i: 2, shape: torch.Size([64, 64, 32, 3, 3]), GNSR = -0.2448
i: 3, shape: torch.Size([64, 64]), GNSR = -0.2517
i: 4, shape: torch.Size([64, 128, 9216]), GNSR = -1.1143
i: 5, shape: torch.Size([64, 128]), GNSR = -0.4103
i: 6, shape: torch.Size([64, 10, 128]), GNSR = -5.9724
i: 7, shape: torch.Size([64, 10]), GNSR = -1.1094
'''
...
This will separate state (the parameters) from the model and turn the model
into a pure function-
"""
fmodel, params, buffers = make_functional_with_buffers(model)

# Let’s review the changes - first, the model has become the stateless
...
# And the model parameters now exist independently of the model, stored as
# a tuple-
for x in params:
    print(f"shape: {x.shape}")
    
print(f"type(param): {type(params)}")
# type(param): <class 'tuple'>
...
'''
Now, let’s use functorch’s 'grad' to create a new function that computes
...
# We can double check that the results using functorch grad and vmap match
# the results of hand processing each one individually-
...
mutation of arbitrary Python data structures, but it is able to handle
many in-place PyTorch operations.
'''
...
(Ampere) where we’ve seen up to 25x speedups on this example.
"""
...
"""
Per-sample-grads without vmap <torch.utils.benchmark.utils.common.Measurement object at 0x7efdae2e51e0>
compute_sample_grads(data, targets)
  223.06 ms
  1 measurement, 100 runs , 1 thread
Per-sample-grads with vmap <torch.utils.benchmark.utils.common.Measurement object at 0x7efdae2e5180>
...
#  Performance delta: 56.9065 percent improvement with vmap 
